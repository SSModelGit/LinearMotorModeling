...
model SimSysWC "Simple System Model expanded to multiple solenoids, using connectors"
  ...
  SolenoidSystemComponents.MagSen sensor1(threshold=1, Px=2);
  SolenoidSystemComponents.MagSen sensor2(threshold=1, Px=4);
  SolenoidSystemComponents.MagSen sensor3(threshold=1, Px=6);
...
equation 
  ...
  sensor1.command=sol1.command "Tell solenoid when to turn on";
  sensor2.command=sol2.command "Tell solenoid when to turn on";
  sensor3.command=sol3.command "Tell solenoid when to turn on";
  channel.t=time;
  c.v=channel.v;
